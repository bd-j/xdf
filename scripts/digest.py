...
def get_cat_dtype():
    ...
    coldesc = [("id", "S20"), ("region", np.int), ("csig", np.float)]
...
if __name__ == "__main__":
    ...
    vers = "v0"
    names = glob.glob("../odyresults/{}/*pkl".fromat(vers))
    ...
    for n in names:
        ...
        csig = colors.std(axis=-1)
        rows.append([make_row(stat, ctype, id=i, region=reg, csig=cs)
                     for stat, i, cs in zip(stats, ids, csig)])
