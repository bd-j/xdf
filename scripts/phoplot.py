...
def plot_seds(sampler, start=0):
    ...
    for isource, source in enumerate(truth):
        ...
        ax.text(0.5, 0.75, "Source #{}".format(int(isource+1)), transform=ax.transAxes, fontsize=10)
...
def hmc_movie(sampler, iterations, stamps, pars_to_show, outname="movie_test.pdf",
    ...
    maxstep = np.sum([len(sampler.trajectories[i]) for i in range(np.max(iterations))])
    pinds = pars_to_show
    ...
    for k, i in enumerate(iterations):
        fig, axlist = hmc_movie_frame(sampler, i, stamps, show_trajectories=traj,
...
if __name__ == "__main__":
    ...
    if True:
        ...
        hmc_movie(sampler, iterations, stamps_to_show, pinds, label=label, outname=outname, png=True)
